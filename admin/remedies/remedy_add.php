...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $shelflife = mysqli_real_escape_string($conn, $_POST['shelflife'] ?? '0');
    ...
    if ($featured == 1) {
        ...
        if (empty($advantages)) {
            $errors['advantages'] = 'Advantages are required';
        }
        if (empty($disadvantages)) {
            $errors['disadvantages'] = 'Disadvantages are required';
        }
    } else if ($featured == 0) {
        ...
        if (empty($how_to_make)) {
            $errors['how_to_make'] = 'Method is required';
        }
        if (empty($advantages)) {
            $errors['advantages'] = 'Advantages are required';
        }
        if (empty($disease_id) || $disease_id == '0') {
            $errors['disease_id'] = 'Disease is required';
        }
    }
    ...
    if (empty($errors)) {
        ...
        $insertSql = "INSERT INTO `remedies` (
            `name`,
            `price`,
            `introduction`,
            `advantages`,
    ...
    <div class="main-content">
        ...
        <main>
            <div class="page-content">
                <div class="card">
                    <h2 class="page-heading">Add Remedy</h2>

                    <form class="page-form" action="remedy_add.php" method="POST" enctype="multipart/form-data">
                        <fieldset>
                        ...
                        <fieldset style="display: inline-block;">
                            ...
                            <input type="radio" id="featured_no" name="featured" value="0" onchange="featured_change(this)" <?php if ($featured == 0) { echo 'checked';} ?>>
                            <label for="featured_no">Remedy</label>
                            <input type="radio" id="featured_yes" name="featured" value="1" onchange="featured_change(this)" <?php if ($featured == '1') { echo 'checked';} ?>>
                            <label for="featured_yes">Herb</label>
                        </fieldset>
                  
                        <fieldset style="display: inline-block;">
                            <legend>ShelfLife:</legend>
                            <input type="radio" id="shelflife_no" name="shelflife" value="0" <?php if ($shelflife == '0') { echo 'checked';} ?>>
                            <label for="shelflife_no">No</label>
                            <input type="radio" id="shelflife_yes" name="shelflife" value="1" <?php if ($shelflife == '1') { echo 'checked';} ?>>
                        ...
                        <fieldset class="fieldset-row Disease">
                            ...
                            <select name="disease_id">
                                ...
                                <?php foreach ($diseasesList as $disease) { ?>
                                    <option value="<?= $disease['id'] ?>" <?= $disease_id == $disease['id'] ? 'selected' : '' ?>>
                                        <?= $disease['name'] ?>
                                    </option>
                                <?php }; ?>
                            </select>
                        ...
                        <fieldset class="fieldset-row ingredients">
                            ...
                            <textarea name="ingredients" class="ingredients-editor" placeholder="Ingredients"><?= $ingredients ?></textarea>
                        ...
                        <fieldset class="fieldset-row How_To_Make">
                            ...
                            <textarea name="how_to_make" class="how_to_make-editor" placeholder="How To Make"><?= $how_to_make ?></textarea>
                            <span class="error"><?php echo $errors['how_to_make'] ?? ''; ?></span>
                        </fieldset>
                        <fieldset class="fieldset-row introduction" style="display: none;">
                            <label for="introduction">Introduction:</label>
                            <textarea name="introduction" class="introduction-editor" placeholder="Introduction"><?= $introduction ?></textarea>
                        ...
                        <fieldset class="fieldset-row Advantages">
                            ...
                            <textarea name="advantages" class="advantages-editor" placeholder="Advantages"><?= $advantages ?></textarea>
                            <span class="error"><?php echo $errors['advantages'] ?? ''; ?></span>
                        </fieldset>
                        <fieldset class="fieldset-row DisAdvantages" style="display: none;">
                            <label for="disadvantages">Disadvantages:</label>
                            <textarea name="disadvantages" class="disadvantages-editor" placeholder="Disadvantages"><?= $disadvantages ?></textarea>
                            <span class="error"><?php echo $errors['disadvantages'] ?? ''; ?></span>
                        </fieldset>
                        <input type="submit" class="form-submit" value="Add Remedy">
                    </form>
                </div>
            </div>
        </main>
